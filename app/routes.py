...
from app.availability import check_url_availability, is_nextory_url
...
def init_routes(config_manager, download_manager, config_dir: str, downloads_dir: str):
        ...
        for url in url_list:
            ...
            if is_nextory_url(url):
                try:
                    ...
                    logger.warning("Could not verify Nextory availability for %s: %s", url, error)
                    ...
                    warnings.append(
                        {
                            ...
                            "message": (
                                f"Skipped unavailable Nextory format(s): {', '.join(unavailable)}"
                            ),
                        }
                    )
    ...
    async def url_availability(url: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail="Could not verify availability with Nextory"
            ) from None
